...
if [ ! -f ~/.wine/drive_c/Python310/Scripts/pip.exe ]; then
  ...
  xvfb-run wine64 ~/.wine/drive_c/Python310/python.exe get-pip.py
  rm get-pip.py
fi

xvfb-run wine64 ~/.wine/drive_c/Python310/python.exe -m pip install --upgrade MetaTrader5 Flask flask-cors
...
if command -v pm2 &> /dev/null; then
  ...
  nohup xvfb-run wine64 ~/.wine/drive_c/Python310/python.exe mt5_bridge.py > bridge.log 2>&1 &
